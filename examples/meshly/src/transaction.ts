/**
 * Meshly SAGA Transaction Coordinator
 * Orchestrates multi-step distributed operations across external systems.
 * If step N fails, executes compensating actions for steps N-1 down to 1 in reverse order.
 */
  ...
  async execute(): Promise<SagaExecutionResult> {
    ...
    for (let i = 0; i < this.steps.length; i++) {
      ...
      if (state.workflowResult === "SUCCESS") {
        ...
        // Run compensation in reverse order for all completed previous steps
