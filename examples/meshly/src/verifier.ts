/**
 * Meshly Reality Engine & Verifier
 * Decouples model claims from physical world state.
 * Emits machine-verifiable EvidenceBundles with tamper-evident SHA-256 state digests.
 */
...
export class Verifier {
  /**
   * Execute an action wrapped in pre- and post-condition verification contracts.
   * Explicitly evaluates:
   *   1. Agent Claim vs Physical Reality
   *   2. Tool Execution Status
   *   3. Observed World State Match
   *   4. Workflow Commit Result
   */
  static async verifyStep(params: {
    ...
    // 1. Observe Pre-State & Verify Preconditions
    ...
    if (!prePassed) {
      ...
      if (events) {
        events.emit("verification.failed", {
          workerId,
          data: { reason: preFailureReason, stage: "precondition" },
        })
      }
    ...
    // 2. Execute Action (and capture agent's claim)
    ...
    // 3. Observe Post-State & Verify Postconditions against Independent Reality
    ...
    if (!postPassed) {
      // Divergence caught: The agent may have claimed success, but world state says otherwise!
      if (contract.compensate) {
    ...
    // 4. Verification Successful -> Generate Verifiable Evidence Bundle
    const stateDiff = {
    ...
    if (events) {
      events.emit("verification.passed", {
        ...
        data: {
          intent: contract.intent,
          digest: tamperEvidentDigestSha256,
        },
      })
    }
