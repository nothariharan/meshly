...
export class Scheduler {
  ...
  calculateScore(worker: Worker): ScheduleCandidate {
    ...
    const reasons: string[] = [`Base priority: ${worker.priority}`]

    if (worker.deadline) {
      const msLeft = worker.deadline.getTime() - Date.now()
      if (msLeft <= 0) {
        score += 150
        reasons.push("Deadline overdue (+150)")
      } else if (msLeft < 60_000) {
        score += 100
        reasons.push("Deadline < 1m (+100)")
      } else if (msLeft < 5 * 60_000) {
        score += 50
        reasons.push("Deadline < 5m (+50)")
      }
    }
    ...
    if (budgetMargin <= 0) {
      score -= 200
      reasons.push("Budget exhausted (-200)")
    } else if (budgetMargin < 0.2) {
      score -= 30
      reasons.push("Low budget margin (-30)")
    }
