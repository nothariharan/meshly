 ...
 * Harmonizes all three Solari execution primitives:
...
export async function runReconciliationWorkflow(mesh: Meshly): Promise<ReconciliationReport> {
  ...
  const rootAuthority = AuthorityManager.issue({
  ...
  mesh.runtime.complete(chiefWorker.id)
...
runReconciliationWorkflow(mesh)
  .then(() => process.exit(0))
  .catch((err) => {
